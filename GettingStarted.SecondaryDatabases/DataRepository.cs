...
namespace GettingStarted.SecondaryDatabases
{
  public abstract class Repository: IDisposable
  {
    ...
    {
      WriteLine("calling null secondary function...");
      return null;
    }
    ...
    protected void AddToDb(string keyval, byte[] dataval)
    {
      ...
      {
        Sync();
        WriteLine($"{DateTime.Now}\t{ex.GetType()} in {db.DatabaseName} put: {ex.Message}");
      }
    }
    ...
    public void DeleteRecordUsingCursor(string keyval)
    {
      ...
      while (b)
      {
        ...
        {
          WriteLine($"key found: '{keyval}'. Deleting...");          
          cursor.Delete();          
          WriteLine($"database record '{keyval}' deleted.");          
          break;
        }
      ...
      watch.Stop();
      WriteLine($"delete task completed in {watch.ElapsedMilliseconds} milliseconds.");
      WriteLine("---");
    }
    ...
    protected void ModifyRecordUsingCursor(string keyval, byte[] dataval)
    {
      ...
      while (b)
      {
        if (Encoding.UTF8.GetString(cursor.Current.Key.Data) == keyval)
        {
          WriteLine($"key found: '{keyval}'. Modifying...");
          cursor.Current.Value.Data = dataval;          
          WriteLine($"database record '{keyval}' modified.");          
          break;
        }
      ...
      watch.Stop();
      WriteLine($"modify task completed in {watch.ElapsedMilliseconds} milliseconds.");
      WriteLine("---");
    }
    
    public void Sync()
    {
      WriteLine("I'm syncing!");
      db.Sync();
      indexDb.Sync();
    }
